...
function showPopup(content) {
    popupInfo.innerHTML = content; // Agregar contenido dinámico
    popup.style.display = 'flex'; // Hacer visible el pop-up
    setTimeout(() => popup.classList.add('show'), 10); // Agregar animación con retardo
}

function closePopupHandler(event) {
    if (event.target === popup || event.target === closePopup) {
        popup.classList.remove('show'); // Quitar clase animada
        setTimeout(() => popup.style.display = 'none', 300); // Esperar animación antes de ocultar
    }
}
